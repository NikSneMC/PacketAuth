...
public class MigrateConfig {
    ...
    public MigrateConfig(ConfigManager config, ConfigManager tokens) {
        if (config.containsKey("config.version") && config.getString("config.version").equals("1.6")) return;
        ...
        tokens.overwrite(config.asMap());
        config.clear();
        config.putString("config.version", "1.6");
